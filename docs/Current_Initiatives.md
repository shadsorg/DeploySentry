...
| Flag Lifecycle + Org Audit & Undo | Design | [Spec](./superpowers/specs/2026-04-30-flag-lifecycle-and-org-audit-design.md) | Three deliverables: (1) tighten archive UX with typed-slug confirmation, scope Danger Zone to Settings tab — frontend, in flight on `feature/flag-archive-ux`. (2) add retention-gated hard delete (`delete_after`, restore, sweep job, migration 060). (3) org-level Audit page at `/orgs/:slug/audit` with old/new diff and one-click revert backed by a revertibility registry. |
