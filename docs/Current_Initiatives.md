...
| E2E SDK Flag Delivery — remaining | Implementation | [Spec](./superpowers/specs/2026-04-12-e2e-sdk-flag-delivery-design.md) | Main suite merged in PR #34 (2026-04-12). React probe re-enabled in Scenario A (root cause: react-harness vite config aliased `@dr-sentry/react` to a path that no longer existed once the SDK switched to dual ESM/CJS via `exports`). 2026-05-01: fixed latent React SDK bug — `useFlagSnapshot`'s `getSnapshot` returned a fresh `Array.from(...)` per call, infinite-looping `useSyncExternalStore` and tripping React error #185 the moment the React tree re-rendered. Fix caches `flagsSnapshot` on `DeploySentryClient` and rebuilds it inside `emit()`; locked in with a Jest assertion. Full 5/5 SDK suite green locally on `feature/e2e-react-probe-fix`. Still pending: fault injection (manual one-time verification per spec §"Testing the Tests"), required-gate flip in branch protection (after a soak). |
