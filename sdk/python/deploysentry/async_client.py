"""Asynchronous DeploySentry client."""
...
from .streaming import AsyncSSEClient

logger = logging.getLogger("deploysentry.async_client")


class AsyncDeploySentryClient:
    """Asynchronous client for the DeploySentry feature-flag service.
    ...
    async def initialize(self) -> None:
        ...
        self._initialized = True
        logger.info("AsyncDeploySentryClient initialised (project=%s, env=%s)", self._project, self._environment)
    ...
    async def __aenter__(self) -> "AsyncDeploySentryClient":
    ...
    async def _fetch_flags(self) -> None:
        ...
        if self._environment:
            params["environment"] = self._environment
        resp = await self._http.get("/api/v1/flags", params=params)
    ...
    async def _evaluate(self, key: str, context: Optional[EvaluationContext] = None) -> Optional[EvaluationResult]:
        """Evaluate a single flag via the API, with caching."""
        ...
        try:
            resp = await self._http.post("/api/v1/flags/evaluate", json=body)
            resp.raise_for_status()
    ...
    async def bool_value(self, key: str, default: bool = False, context: Optional[EvaluationContext] = None) -> bool:
        """Evaluate a boolean flag."""
        result = await self._evaluate(key, context)
        if result is None:
            return default
    ...
    async def string_value(self, key: str, default: str = "", context: Optional[EvaluationContext] = None) -> str:
    ...
    async def int_value(self, key: str, default: int = 0, context: Optional[EvaluationContext] = None) -> int:
    ...
    async def json_value(self, key: str, default: Any = None, context: Optional[EvaluationContext] = None) -> Any:
        """Evaluate a flag that holds a JSON value."""
        result = await self._evaluate(key, context)
        if result is None:
            return default
    ...
    async def detail(self, key: str, context: Optional[EvaluationContext] = None) -> EvaluationResult:
