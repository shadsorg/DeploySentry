...
export function newProvisionalId(): string {
  ...
  // Set the provisional variant: top two bits of byte 8 = 11, so the
  // canonical-string char at index 19 is always in [c-f].
...
// provisional means that nibble has its top two bits set (c, d, e, or f).
export function isProvisionalId(id: string): boolean {
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[c-f][0-9a-f]{3}-[0-9a-f]{12}$/.test(id);
}
