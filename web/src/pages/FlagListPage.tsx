...
export default function FlagListPage() {
  ...
  // Optimization: Memoize flag filtering and hoist search string allocation outside the loop
  // to avoid O(N) penalties per render. Moved before early returns to satisfy rules of hooks.
  const filtered = useMemo(() => {
    const q = search?.toLowerCase() ?? '';
    return flags.filter((flag) => {
      if (q && !flag.name.toLowerCase().includes(q) && !flag.key.toLowerCase().includes(q)) {
        return false;
      }
