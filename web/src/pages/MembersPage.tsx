...
export default function MembersPage() {
  ...
  // ⚡ Bolt: Memoize visible members filtering to prevent O(N) recalculations on every render
  const visibleMembers = useMemo(() => {
    return roleFilter === 'all' ? members : members.filter((m) => m.role === roleFilter);
  }, [members, roleFilter]);

  // ⚡ Bolt: Pre-compute role counts in O(N) instead of recalculating O(N) inside the map
  ...
  // ⚡ Bolt: Memoize available group members calculation at the top level to avoid
  // O(N) Set creation and filtering on every render cycle when viewing group details.
  const availableGroupMembers = useMemo(() => {
    if (!groupMembers || !members) return [];
